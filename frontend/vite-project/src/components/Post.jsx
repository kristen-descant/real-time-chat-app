    ...
    <>
      {!post_id ? (
        <div className="flex flex-col border-2 rounded border-[black] w-[50%] items-center hover:bg-color_palette_1 relative">
          ...
          <img className='absolute top-1 right-1 h-6 w-6' src={trash} onClick={()=> deletePost()}/>
          <div className="absolute bottom-1 left-1">Edited : {post.edited.toString()}</div>
          {userInfo && (<button className="border-2 rounded p-1 absolute bottom-1 right-1 disabled:border-0 disabled:text-color_palette_3 hover:bg-color_palette_1" disabled={post.created_by != userInfo.data.id} onClick={() => {setEditing(true)}}>Edit Post</button>)}
          {editing && (
            <form
        ...
        <div className="flex flex-col border-2 border-[black] w-[50%] items-center relative">
          ...
          {!currentPost ? (
            ...
            <div className="flex flex-row">
              <span className="mr-2 text-[green]">{currentPost.up.length}</span>
                <img
                  className="h-3 md:h-5"
                  src={thumbsUp}
                  alt="thumbs up"
            ...
            <span className="mr-2 text-[red]">{currentPost.down.length}</span>
            <img className="h-3 md:h-5" src={thumbsDown} alt="thumbs down" 
            onClick={(e) => downVote(e)}/>
          </div>)
            }
          ...
          <img className='absolute top-1 right-1 h-6 w-6' src={trash} onClick={()=> deletePost()}/>
          {userInfo && (<button className="border-2 rounded p-1 absolute bottom-1 right-1 disabled:border-0 disabled:text-color_palette_3" disabled={currentPost.created_by != userInfo.data.id} onClick={() => {setEditing(true)}}>Edit Post</button>)}
          {editing && (
            <form
            className="flex flex-col overflow-auto"
            onSubmit={(e) => editPost(e)}
